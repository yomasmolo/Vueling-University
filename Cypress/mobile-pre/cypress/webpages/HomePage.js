...
export class HomePage {
  ...
  hamburger = {
    menuBurger: () => cy.get(`[class="header-left flex-1 hamburger hamburger--arrow ng-star-inserted"]`),
    languageSpan: () => cy.get(`#menuLanguage .left-menu-arrow`),
    selectLanguage: () => cy.get(`.item-box`),
    currencySpan: () => cy.get(`#menuCurrencies .left-menu-title`),
    selectCurrency: () => cy.get(`.item-box`),
    closeMenu: () => cy.get(`[class="header-left flex-1 hamburger hamburger--arrow ng-star-inserted is-active"]`),
  };
  ...
  acceptCookies() {
    this.overlap.cookiesBtn().click().should("be.visible");
  }
  closePromo() {
    this.overlap.body().then((body) => {
      if (body.find('[class="splash-dialog"]').length > 0) {
        this.overlap.closeImg().click();
      }
    });
  }
  ...
  selectLanguage() {
    this.hamburger.menuBurger().click();
    this.hamburger.languageSpan().click();
    this.hamburger.selectLanguage().contains("Català").click();
  }

  selectCurrency() {
    this.hamburger.menuBurger().click();

    if (this.hamburger.currencySpan().contains(" Euro ")) {
      this.hamburger.closeMenu().click();
    } else {
      this.hamburger.currencySpan().click();
      this.hamburger.selectCurrency().contains("Euro").click();
    }
  }
